...
export const defaultResponseAdapter = <T>(response: unknown): ApiResponse<T> => {
  ...
  if (typeof response !== 'object') {
    console.warn(
      '[tableUtils] Unrecognized response format. Supported formats include: arrays, objects with ' +
        recordFields.join('/') +
        ' fields, and nested data objects. Current format:',
      response
    )
    return { records: [], total: 0 }
  }
  ...
  if (!recordFields.some((field) => field in res) && records.length === 0) {
    console.warn('[tableUtils] Unrecognized response format')
    console.warn('Supported fields: ' + recordFields.join(', '), response)
    console.warn('Add custom fields in utils/table/tableConfig')
  }
