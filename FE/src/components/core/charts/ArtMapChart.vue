<template>
  <div class="relative w-full" :style="{ height: 'calc(100vh - 120px)' }">
    <div v-if="isEmpty" class="h-full flex-cc">
      <ElEmpty description="No map data available" />
    </div>
...
<script setup lang="ts">
        ...
        formatter: ({ data }: { data?: Record<string, unknown> }) => {
          ...
          return `
            <div style="padding: 8px;">
              <div><strong>Name:</strong> ${name || 'Unknown region'}</div>
              <div><strong>Code:</strong> ${adcode || 'N/A'}</div>
              <div><strong>Level:</strong> ${level || 'N/A'}</div>
            </div>
          `
        }
      },
          ...
          ? [
              {
                ...
                data: [
                  ...
                  { name: 'Shanghai', value: [121.472644, 31.231706, 100] },
                  { name: 'Shenzhen', value: [114.085947, 22.547, 100] }
                ]
              }
            ]
          : [])
  ...
  const handleMapClick = (params: Record<string, unknown>) => {
    if (params.componentType === 'series') {
      ...
      console.log(`Selected region: ${params.name}`, params)
